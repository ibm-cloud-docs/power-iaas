...
To download an ISO image from the ESS website, complete the following steps. For more information, see [Handling AIX fileset](https://www-01.ibm.com/support/docview.wss?uid=ibm10871636){: external}. You can open an SF case against AIX support if you run into any issues.
...
1. Use the [`installp`](https://www.ibm.com/support/knowledgecenter/ssw_aix_72/i_commands/installp.html) command to install the image in a compatible installation package. You must use the following command syntax, `installp -agXd path  software_to_install`.
