...
The {{site.data.keyword.powerSys_notm}} service now includes a highly available 5 Gbps connection to the IBM public cloud services. The 5 Gbps connect is set up in an HA configuration at $0 for each customer per datacenter. If desired, you can select the global routing option for these links at no cost. This change allows for the customer to set up connectivity between the {{site.data.keyword.powerSys_notm}} and the IBM Cloud resources without adding any additional costs. Over the next few months, the {{site.data.keyword.powerSys_notm}} service service plans to continue to evolve its network connectivity capabilities through automation and integration.
