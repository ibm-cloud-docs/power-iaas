...
You can export only a single VM image at a time per {{site.data.keyword.powerSysFull}} instance. If you attempt another operation when one operation is still in progress, an error is displayed. You can use the {{site.data.keyword.powerSysFull}} legacy REST APIs for Image Import until 1st October 2022. You must plan to transition to the new {{site.data.keyword.powerSysFull}} REST APIs or CLI commands within the sunset period.
{: note}
