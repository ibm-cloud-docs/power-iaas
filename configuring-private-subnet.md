...
The first IP address is always reserved for the gateway in all colocations (colo). The second and third IP addresses are reserved for gateway high-availability (HA) in only the WDC04 colo. The subnet address and subnet broadcast address are reserved in both colos.
{: important}
