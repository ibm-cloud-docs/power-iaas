...
The SPP helps you to manage CPU cores only. Pricing for memory and storage remains the same as earlier. The total estimated cost page does not show the SPP reserved cores-related costs because of service-level estimator limitations. For more information about pricing, see [Pricing for {{site.data.keyword.powerSys_notm}}](/docs/power-iaas?topic=power-iaas-pricing-virtual-server-on-cloud). For more information about pricing for SPP in IBM {{site.data.keyword.powerSys_notm}} Private Cloud, see [FAQ]
{: note}
...
3. In the **Create new shared processor pool** window, define the following preferences based on your requirements:
    ...
    |Reserved processing cores|[On Cloud]{: tag-blue} For {{site.data.keyword.powerSys_notm}} on cloud, the core-to-vCPU ratio is 1:1 by default. \n [Private Cloud]{: tag-red} For IBM {{site.data.keyword.powerSys_notm}} Private Cloud, the core-to-vCPU ratio can be 1:20. The minimum entitled capacity must be 0.05 and can be incremented by 0.05.|
