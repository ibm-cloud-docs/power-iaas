...
To use an SSH key with a VM-create operation, you must add the public key to the {{site.data.keyword.powerSysFull}} instance by using the [`ibmcloud pi ssh-key create KEY_NAME --key KEY`](/docs/power-iaas?topic=power-iaas-power-iaas-cli-reference-v1#ibmcloud-pi-ssh-key-create) command.




To add the generated public key and to set its visibility by using the `visibility` attribute, enter the following command (by replacing the example value with your own public key) :
...
## Setting the visibility of an SSH key in a {{site.data.keyword.powerSys_notm}} workspace
{: #scope-ssh}

In a {{site.data.keyword.powerSys_notm}} workspace, when you create the SSH key, you can set the visibility of an SSH key as `Workspace` or `Account`. When you set the visibility of an SSH key as `Workspace`, the SSH key is available only in the workspace where it is created and not available in the other workspaces. When you set the visibility of an SSH key as `Account`, the SSH key is available in all the workspaces under the account. However, you can delete the SSH key only from the workspace in which it is created. Therefore, the security and privacy of the SSH key is improved.
{: shortdesc}
...
The SSH key is created and listed in the data table on the SSH key page. The **Access setting** column in the data table displays the visibility of the SSH key as `Workspace` or `Account`.
