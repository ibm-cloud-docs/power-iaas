...
* You can choose to enable or disable notifications for events on all workspaces. For more information, see [Customizing the log notification settings](/docs-draft/power-iaas?topic=power-iaas-manage-event-logs#set-event-logs).
...
1.	Access the **Event logs** page from the user interface. For more information, see [Accessing the event logs](/docs-draft/power-iaas?topic=power-iaas-manage-event-logs#access-event-logs).
