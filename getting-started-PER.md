...
To use PER, you need to create a new workspace using the PER enabled data centers or migrate your existing workspace to PER.

To perform the migration, complete the following steps:
1.	Create a [case](https://cloud.ibm.com/unifiedsupport/cases/form){: external} under **Customer Success** from the **Support Center** of IBM Cloud.
2.	Select **Upgrade product/service** under Subtopic and click **Next**.
3.	Provide a short description of your requirements.
4.	In the **Description** box under additional information provide the following information:
  1.  The _CRN_ of your workspace that you want to upgrade.
  2.	The list of _subnets_ configured in the workspace.
  3.	The list of _Direct Link_ connections to which the subnets are attached.  
  The {{site.data.keyword.powerSys_notm}} operation team will process the support ticket by configuring the PER and other network devices (in parallel to the Direct Link configuration). When the configuration is complete, you are notified through ticket update that the PER configuration is ready for your validation and testing.  

5.	Schedule a maintenance window, during which you need to provision the Transit Gateway to conclude the PER network construction. To force the network to go through PER network, perform the following steps: 
  1.	From the IBM Cloud catalog, browse your provisioned Direct Link connections.
  2.	Click on a Direct Link connection to open the connection details page.
  3.	Click on the **BGP** tab and then open the **Route Filter** details.
  4.	Select **Deny all import routes** to block the Direct Link routes and force the traffic to go through the PER network.  
  If you face any PER connectivity problem and wish to revert to Direct Link path, select **Permit all import routes** to unblock the Direct Link routes and disconnect the workspace from the Transit Gateway.

6.	Upon successful testing, delete the Direct Link connections and notify IBM by updating the ticket.
7.	The {{site.data.keyword.powerSys_notm}} team will perform in-house cleanup, mark the workspace as “Migrated”, and close the ticket.  
