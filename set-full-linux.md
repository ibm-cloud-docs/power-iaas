...
2. Open the Security groups for VPC page by navigation to IBM Cloud dashboard > VPC Infrastructure > Networks > Security groups. For more information on security groups, see [security groups](/docs/vpc?topic=vpc-using-security-groups). 

3. Select the security group that is attached to your proxy and add these port numbers: 443, 8443, 80, and 3128. Ensure that the IP being asked is the subnet CIDR of your private network of Power Systems Virtual Server instance. To view the subnet CIDR in the IBM Cloud dashboard go to the zone, click **Subnets**, identify the subnet that you will be using, and copy the CIDR.

4. After the proxy is created, log in to the proxy instance and locate the subnet CIDR by completing the following steps:
  
   a. Open command-line interface (CLI) and run the `ssh root@<public IP of proxy>` command.
...
2.	In the proxy instance CLI, start an SSH connection to your RHEL or SLES PVM instance by using the PVM’s external IP address by using the following command:

      -	`ssh root@<external IP address>`  
      
        Example: ssh root@1.2.3.4

3.  Add a network route from the PVM instance to the proxy instance that was configured in [step 2](/docs/power-iaas?topic=power-iaas-set-full-Linux#create-proxy).

      - Run the `ip route` command and add CIDR subnet of proxy through private gateway of PVM.
        ...
        Example: Run `ip route` command and add the CIDR subnet IP 1.5.3.9/24 through private gateway IP 5.9.7.1.
...
1. Log in to your proxy instance, by using the `ssh root@<public IP of proxy>` command.
2. Ensure that another IP route is created within the proxy instance. This IP route allows the IPs that are within the range of the created private IP gateway for your VM to this proxy gateway.
...
3. A connection is now established with the proxy and the PVM. You can ping the connection both ways. 
...
5. Set up squid by completing the following steps:
   ...
   You must have root authority to run these commands. After the installation, the squid config file is stored in the `/etc/squid/squid.conf location`. 
...
3. Run the cloud-init script that is located in the `/usr/local/bin/` location. Run the script as _root_. You can switch the user to _root_ by using the `sudo su root` command.
   ...
   a. For	RHEL, run the following command:
    ...
    b. For SLES, run the following command: 
      ...
      -s = the URL of the SLES Repository Mirroring Tool (RMT) server you are registering against
