...
- Put your virtual machine (VM) on the public network. You can add a public network [during](/docs-draft/power-iaas?topic=power-iaas-creating-power-virtual-server#configuring-instance) or [after](/docs-draft/power-iaas?topic=power-iaas-modifying-instance#adding-removing-network) the provisioning stage. Depending on your VM, see the [SUMA tasks and the command line](#suma-tasks-cli) section for information on the **suma** command or the [SNDPTFORD command](#sndptford-command).
- Set up another VM as an AIX [NIM server](/docs-draft/power-iaas?topic=power-iaas-provisioning-nim) or an IBM i [Network installation Server](#ibmi-network-server).
...
Before you can install or upgrade an IBM i system through the network, you must [set up a network installation server](/docs-draft/power-iaas?topic=power-iaas-preparing-install-server). The network installation server contains images of the IBM i operating system, its internal code as well as licensed programs and PTFs.
