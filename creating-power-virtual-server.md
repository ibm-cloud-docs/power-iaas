...
12. Define your **Network interfaces** by adding a public network, private network, or both. Public networking is not available in all the IBM data center locations. When you add an existing private network, you can choose a specific IP address or have one auto-assigned.
...
To deploy a {{site.data.keyword.powerSys_notm}} VSI specify any name. To delete a VSI and to deploy a new VSI with the same name, allow up to 1 hour between deletion of the original instance and creation  a VSI with the same name.
