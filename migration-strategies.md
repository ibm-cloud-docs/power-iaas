...
- [Configuring IBM Cloud Mass Data Migration (MDM) on AIX VM](/docs/allowlist/power-iaas?topic=power-iaas-configuring-mdm)
- [Configuring Mass Data Migration (MDM) on IBM i VM](/docs/allowlist/power-iaas?topic=power-iaas-configuring-mass-data-migration-mdm-on-ibm-i-vm) -->
...
3. [Deploying a custom image within a {{site.data.keyword.powerSys_notm}}](/docs/allowlist/power-iaas?topic=power-iaas-deploy-custom-image).
...
- [*Falconstor StorSafe VTL*](/docs/allowlist/power-iaas?topic=power-iaas-migration-strategies-power#falconstor-storsafe-vtl)
- *iCluster*
- *MIMIX from Syncsort*
...
For more information, see [Restoring an AIX mksysb image onto a {{site.data.keyword.powerSys_notm}} instance](/docs/allowlist/power-iaas?topic=power-iaas-restoring-aix-mksysb-image).
...
1. Define the VM as a NIM master. For more information, see [Setting up a Network Installation Management (NIM) server](/docs/allowlist/power-iaas?topic=power-iaas-provisioning-nim).
2. Create a License Program Products (LPP) source by using an image repository that contains the images. Stock images already contain the ISO and LPPs that are found on the base media, so create the LPP source and NIM resources from [Installing optional software products from the AIX stock image](/docs/allowlist/power-iaas?topic=power-iaas-using-ess-iso#installing-aix-stock-image).
